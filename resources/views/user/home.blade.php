...
<body>
    <article class="leaderboard">
        <header>
            ...
            <h1 class="leaderboard__title"><span class="leaderboard__title--top">Event</span><span
                    class="leaderboard__title--bottom">List</span></h1>
        </header>

        <main class="leaderboard__profiles">
            @foreach ($events as $event)
                <a class="no_decor" href="{{ route('event', ['slug' => $event->slug]) }}">
                    <article class="leaderboard__profile">
                        <img src="{{ optional($event->image)->thumbnail_url ?? 'https://via.placeholder.com/100' }}"
                            alt="{{ $event->title }}" class="leaderboard__picture">
                        <span class="leaderboard__name">{{ $event->title }}</span>
                        <span
                            class="leaderboard__value {{ strtolower($event->status) }}"><span>{{ $event->status }}</span></span>
                    </article>
                </a>
            @endforeach
